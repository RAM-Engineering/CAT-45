...
The performance of the CAT-45 engine is fundamentally governed by the thermochemical and physical properties of the selected solid propellant. In accordance with classical solid rocket motor analysis as presented by Sutton, the characteristic velocity $c^*$, specific impulse $I_{sp}$, combustion temperature $T_c$, and ratio of specific heats $\gamma$ together determine achievable chamber pressure, mass flow rate, and nozzle performance. For this design, a potassium nitrate and sorbitol (KNSB) formulation is selected due to its well documented combustion behavior, moderate flame temperature, and suitability for small scale cast grain applications.

\subsection{Potassium Nitrate and Sorbitol Propellant}


The CAT-45 motor employs the KNSB composite solid propellant in a nominal 65/35 oxidizer to fuel mass ratio. This formulation is widely documented in experimental rocketry literature and provides analytically tractable combustion behavior suitable for preliminary engineering analysis. Potassium nitrate (KNO$_3$) functions as the oxidizer, supplying oxygen for the combustion of the fuel component, sorbitol (C$_6$H$_{14}$O$_6$). Upon heating to ignition temperature, the mixture undergoes a condensed phase decomposition followed by exothermic gas phase reactions that generate high temperature combustion products consisting of both gaseous species and condensed phase particulates, primarily potassium carbonate and other alkali compounds. The thermochemical properties adopted for analysis in this document are consistent with published equilibrium calculations and experimental measurements for KNSB propellant at pressures near 1000 psi. Representative values used throughout the CAT-45 design are:

\[I_{sp,\text{ideal}} \approx 151.5~\text{s}, c^* \approx 908~\text{m/s}, \gamma \approx 1.137, M \approx 39.9~\text{g/mol}, T_c \approx 1327~\text{K} \]

The relatively low characteristic velocity compared to other propellants such as composite ammonium perchlorate propellants reflects the lower combustion temperature and significant fraction of condensed phase products. For KNSB, the condensed phase mass fraction is approximately:

\[X \approx 0.436\]

which has important implications for nozzle performance and erosion behavior. The propellant is cast into a cylindrical grain using melt casting techniques. Sorbitol serves as both fuel and binder, forming a solid matrix upon cooling. The density of the cast propellant is approximately:

\[\rho \approx 1.82~\text{g/cm}^3\]

which is used in grain sizing calculations.

\subsection{Combustion Behavior and Thermal Considerations}

The combustion behavior of KNSB propellant follows the classical pressure dependent burning rate law for solid propellants:

\[r = a p^n\]

where $r$ is the linear regression rate, $p$ is chamber pressure, and $a$ and $n$ are empirical constants. For KNSB propellant, the burn rate at atmospheric pressure is approximately:

\[r_{1~\text{atm}} \approx 2.6~\text{mm/s}\]

and at 1000 psi is approximately:

\[r_{1000~\text{psia}} \approx 11.3~\text{mm/s}\]

These values indicate moderate pressure sensitivity and stable combustion behavior above approximately 400 psi. The selected operating chamber pressure of:

\[p_c = 3.447 \times 10^6~\text{Pa} \quad (500~\text{psi})\]

lies within the regime of reliable and repeatable combustion performance documented in experimental practice.The combustion temperature of approximately 1327 K is significantly lower than that of typical composite propellants, reducing thermal loading on the nozzle and casing. However, the presence of condensed phase particles introduces two important considerations:

\begin{enumerate}
\item Reduced effective exhaust velocity relative to purely gaseous equilibrium assumptions.
\item Increased potential for erosive interaction with nozzle surfaces.
\end{enumerate}

\subsection{Suitability for Graphite Nozzle Applications}

Graphite is selected as the nozzle material for CAT-45 due to its high temperature capability, machinability, and resistance to thermal shock. The moderate combustion temperature of KNSB propellant reduces the risk of rapid thermal degradation relative to higher temperature composite propellants. ****Although KNSB produces condensed phase products, graphite exhibits favorable erosion resistance in small solid rocket motor applications operating at chamber pressures.**** [need to double check source]. Nozzle divergence losses have already been incorporated into the thrust coefficient through the factor

\[C_F = 1.49\]

and additional viscous and two phase losses are conservatively accounted for by adopting the effective specific impulse

\[
I_{sp,\text{eff}} \approx 138~\text{s}
\]

for internally consistent thrust sizing. The combination of moderate chamber pressure, moderate flame temperature, and conservative thrust modeling renders KNSB propellant compatible with a graphite conical nozzle for the intended five second burn duration. For a first generation educational motor, this material pairing provides a favorable balance between performance, manufacturability, and structural margin.
