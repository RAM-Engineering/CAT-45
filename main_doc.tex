...
where $r_i$ is the internal radius and $t$ is the wall thickness. For 4130 chromoly steel, a common material for rocket motor casings:
\begin{itemize}
\item Yield strength: $\sigma_y = 435$ MPa (63,000 psi)
\item Ultimate tensile strength: $\sigma_{uts} = 560$ MPa (81,000 psi)
\end{itemize}

Applying a safety factor of 3 based on yield strength (conservative for 
educational and first-motor applications):
\[
\sigma_{\text{design}} = \frac{\sigma_y}{3} = \frac{435}{3} = 145~\text{MPa}
\]

Rearranging the hoop stress equation to solve for required wall thickness:
\[
t = \frac{p_c r_i}{\sigma_{\text{design}}}
\]

For preliminary analysis, assume an internal diameter of approximately 
44 mm (to be verified in Section 8 based on grain geometry), giving 
$r_i = 22$ mm. At a chamber pressure of 500 psi (3.447 MPa):
\[
t = \frac{3.447 \times 10^6 \times 0.022}{145 \times 10^6} = 0.52~\text{mm}
\]
...
\]
is specified. This provides an effective safety factor of approximately 3.8 relative to the theoretical minimum, or a safety factor of 11.4 relative to material yield strength. At 2.0 mm wall thickness and 44 mm internal diameter, the actual hoop stress  during operation is:
\[
\sigma_{\text{actual}} = \frac{3.447 \times 10^6 \times 0.022}{0.002} = 37.9~\text{MPa}
\]

This represents only 8.7\% of the yield strength, providing substantial margin for:
\begin{itemize}
